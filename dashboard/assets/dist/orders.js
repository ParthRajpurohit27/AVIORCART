...
function orderCardHtml(o, index) {
    ...
    const itemsHtml = items
        .map(function (it) {
        return ('<div class="order-item-row"><span>' +
            escapeHtml(it.title) +
            " × " +
            it.quantity +
            "</span><span>" +
    ...
    return ('<div class="order-card reveal" style="--delay:' + delay + 'ms" data-id="' + o.id + '">' +
        ...
        detailRow("Address", escapeHtml(o.address || "-") + ", " + escapeHtml(o.city || "-") + ", " + escapeHtml(o.state || "-") + " - " + escapeHtml(o.pincode || "-")) +
...
function setRefreshing(state) {
    ...
    if (el)
        el.classList.toggle("spinning", state);
}
let toastTimer;
